...
exports.getNewsById = async (req, res, next) => {
  try {
    ...
    const updatedNews = await newsService.increaseNewsViews(id);
...
exports.fetchAndSaveNews = async (req, res, next) => {
  try {
    ...
    if (totalSaved === 0) {
      return res.status(400).json({
        success: false,
        message: 'No news articles fetched. Check your NEWS_API_KEY in .env file or check server logs for errors.',
      });
    }

    res.json({
      success: true,
      message: `News fetched and saved successfully. Total articles: ${totalSaved}`,
      totalSaved,
    });
  } catch (error) {
    next(error);
...
exports.getNewsStats = async (req, res, next) => {
  try {
    ...
    const topCategoryResult = await News.aggregate([
